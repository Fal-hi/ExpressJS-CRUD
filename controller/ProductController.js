  ...
  upload(req, res, async (err) => {
    if (err) {
      ...
      try {
        ...
        const finalImageUrl = `${req.protocol}://${req.get('host')}/uploads/${
          req.file.filename
        }`
...
async function findProductById(req, res) {
  try {
    const result = await models.products.findByPk(req.params.id)
    result
      ? res.status(200).send({ message: result })
...
async function getInfoProductDetails(req, res) {
  try {
